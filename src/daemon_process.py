...
from science_new_client import ScienceNewClient
from notify_wecom import WebHookNotifierClient  # 导入企业微信通知器类，用于发送通知
...
def science_new_job(science_new_client, report_generator, webhook_notifier):
    LOG.info("[开始执行定时任务]科学新闻热点话题跟踪")
    markdown_file_path = science_new_client.export_top_news()
    report, _ = report_generator.generate_science_topic_report(markdown_file_path)
    LOG.info(f"[定时任务执行完毕]")
...
def main():
    ...
    science_new_client = ScienceNewClient(config.science_news_url, config.scienct_news_key)  # 创建 Science News 客户端实例
    notifier = Notifier(config.email)  # 创建通知器实例
    webhook_notifier = WebHookNotifierClient(config.wecom_webhook_url, config.wecom_key)  # 创建Webhook通知器实例
    ...
    # science_new_job(science_new_client, report_generator, webhook_notifier)
    ...
    # 安排 每天执行一次.
    schedule.every(24).hours.at(":00").do(science_new_job, science_new_client, report_generator, webhook_notifier)
